...
def plot_eigvals_symm():
    ...
    plt.scatter(np.real(eigvals),np.imag(eigvals),label='Eigenvalues',linewidths=3)
    plt.scatter(np.real(eigvals_ph),np.imag(eigvals_ph),label='Estimated Eigenvalues Phugoid',linewidths=3)
    plt.scatter(np.real(eigvals_sp),np.imag(eigvals_sp),label='Estimated Eigenvalues Short Period',linewidths=3)
...
def plot_eigvals_asymm():
    ...
    plt.scatter(np.real(eigvals),np.imag(eigvals),label='Eigenvalues',linewidths=3)
    plt.scatter(np.real(eigvals_ap),np.imag(eigvals_ap),label='Estimated Eigenvalues Aperiodic Roll',linewidths=3)
    plt.scatter(np.real(eigvals_dr),np.imag(eigvals_dr),label='Estimated Eigenvalues Dutch roll',linewidths=3)
    plt.scatter(np.real(eigvals_s),np.imag(eigvals_s),label='Estimated Eigenvalues Aperiodic Spiral',linewidths=3)
    plt.xlabel('Real axis')
    plt.ylabel('Imaginary axis')
    plt.grid()
    plt.legend()
