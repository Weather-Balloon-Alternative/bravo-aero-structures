...
Ix = 0.165650732/4 # MMOI around x-axis TODO Get actual value
Iy = 0.156766341/4 # MMOI around y-axis TODO Get actual value
Iz = 0.318083851/4 # MMOI around z-axis TODO Get actual value
nmax = 2.5 # Maximum allowed load factor

## Stability stuff for symmetric
muc = m/(rho*S*c) #relative density [=m/(rho*S*c)]
CZadot = -steady['CFz_q'][-1] #derivative of Cz w.r.t. alpha_dot*c/V0 #TODO Find a better alternative
Cmadot = steady['CMm_q'][-1] #derivative of Cm w.r.t. alpha_dot*c/V0 #TODO Find a better alternative
KY2 = Iy/(m*b) #square of the nondimensional radius of gyration about the Y-axis [=I_y/(m*b)]
CXu = steady['CFx_U'][-1]/(0.5*rho*V0*S) #derivative of X w.r.t. u divided by 1/(0.5*rho*V*S)
CXa = -coeff['CFx_Alpha'][-1] #derivative of CX w.r.t. alpha
CXq = -steady['CFx_q'][-1] #derivative of CX w.r.t. qc/V0
CZ0 = -coeff['CFzo'][-1] #CZ in steady flight
CZu = -steady['CFz_U'][-1]/(0.5*rho*V0*S) #derivative of Z w.r.t. u divided by 1/(0.5*rho*V*S)
CZa = -coeff['CFz_Alpha'][-1] #derivative of CZ w.r.t. alpha
CX0 = -coeff['CFxo'][-1] #CX in steady flight
CZq = -steady['CFz_q'][-1] #derivative of CZ w.r.t. qc/V0
Cmu = steady['CMm_U'][-1]/(0.5*rho*V0*S) #derivative of M w.r.t. u divided by 1/(0.5*rho*V*S)
Cma = coeff['CMm_Alpha'][-1] #derivative of Cm w.r.t. alpha
Cmq = steady['CMm_q'][-1] #derivative of Cm w.r.t. qc/V0
...
CYbdot = -steady['CFy_r'][-1] #derivative of CY w.r.t. beta_dot
...
CYp = -steady['CFy_p'][-1] #derivative of CY w.r.t. p*b/(2*V0)
CYr = -steady['CFy_r'][-1] #derivative of CY w.r.t. r*b/(2*V0)
Clb = steady['CMl_Beta'][-1] #derivative of Cl w.r.t. beta
Clp = steady['CMl_p'][-1] #derivative of Cl w.r.t. p*b/(2*V0)
Clr = steady['CMl_r'][-1] #derivative of Cl w.r.t. r*b/(2*V0)
Cnb = steady['CMn_Beta'][-1] #static directional stability, derivative of Cn w.r.t. beta
Cnp = steady['CMn_p'][-1] #derivative of Cn w.r.t. p*b/(2*V0)
Cnr = steady['CMn_r'][-1] #derivative of Cn w.r.t. r*b/(2*V0)
